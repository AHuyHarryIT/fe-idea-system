...
import { getAdminStatValue, isAdminSubmissionOpen } from '@/features/dashboard/admin/helpers/admin-dashboard'
...
export default function AdminDashboardPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const [statsResponse, submissionsResponse] = await Promise.all([
        ...
        submissionService.getSubmissions({ pageNumber: 1, pageSize: SUBMISSION_SELECT_PAGE_SIZE }),
      ])

      if (!statsResponse.success || !submissionsResponse.success) {
        throw new Error(
          statsResponse.error ?? submissionsResponse.error ?? 'Unable to load admin dashboard.',
        )
      }
  ...
  const reviewBacklog = getAdminStatValue(stats?.totalPendingIdeas ?? stats?.pendingReview)
  ...
  const recentSubmissions = useMemo(
    () =>
      [...(data?.submissions ?? [])]
        .sort(
          (left, right) =>
            getDateTimestamp(right.finalClosureDate) - getDateTimestamp(left.finalClosureDate),
        )
        .slice(0, 4),
    [data?.submissions],
  )
  ...
  const openSubmissionCount = useMemo(
    () => (data?.submissions ?? []).filter((submission: Submission) => isAdminSubmissionOpen(submission)).length,
    [data?.submissions],
  )
  ...
  return (
    <div className="mx-auto w-full max-w-7xl">
      <PageHeader
        ...
        actions={
          <>
            <Link to="/manage/users"><AppButton variant="ghost">User directory</AppButton></Link>
            <Link to="/manage/review"><AppButton>Review queue</AppButton></Link>
          </>
        }
      />
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={FolderKanban} title="Total ideas" value={isLoading ? '...' : `${totalIdeas}`} description="All ideas currently counted by the live admin stats endpoint." accent="blue" meta={`${ideasThisMonth} this month`} />
        <StatCard icon={Users} title="Total users" value={isLoading ? '...' : `${totalUsers}`} description="Accounts available across the platform directory." accent="violet" meta="Directory" />
        <StatCard icon={Tags} title="Categories" value={isLoading ? '...' : `${totalCategories}`} description="Configured idea categories for submission classification." accent="amber" meta={`${totalDepartments} departments`} />
        <StatCard icon={ListChecks} title="Pending review" value={isLoading ? '...' : `${reviewBacklog}`} description="Ideas still waiting for a review decision." accent="emerald" meta={`${ideasWithoutComments} no comments`} />
      </div>
      ...
      <div className="mt-6">
        <AdminRecentSubmissionsSection error={error} recentSubmissions={recentSubmissions} />
      </div>
    </div>
  )
}
